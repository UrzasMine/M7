...
\begin{table}[H]
    \centering
    \begin{tabular}{c|c|c}
        Fläche $A \left(\text{m}^2\right)$& Hydrostatischer Druck $p_H\left(\text{Pa}\right)$ & Geschwindigkeit $v \left(\text{m/s}\right)$\\
        \hline
        $0,020$  & $23$ & $5,2$\\
        $0,019$  & $24$ & $5,3$\\
        $0,018$  & $25$ & $5,5$\\
...
\begin{figure}[H]
    ...
    \caption{Der Auftriebsbeiwert aufgetragen gegen den Widerstandsbeiwert. Die Ausgleichsgeraden wurden mittels \texttt{numpy.polyfit} erstellt. Zusätzlich wurde der Mittelwert aus allen $a$ gebildet und die zugehörige konstante Gerade eingezeichnet.}
    \label{if:AV}
\end{figure}
...
\begin{table}[H]
    \centering
    \begin{tabular}{c|c|c}
        Winkelstellung  $\left(°\right)$ & Auftriebskraft $F_W$ $\left(N\right)$ & Widerstandskraft $F_A \left(N\right)$ \\
        \hline
...
\begin{equation}
    ...
    \label{Fehler-Fläche}
\end{equation}
gegeben. Mit $B = 14 \pm 0,3~\text{cm}$ und $L = 22 \pm 0,3~\text{cm}$ beträgt die Tragfläche des Flügels $S = 308 \pm 0,6~\text{cm}^2$.  
